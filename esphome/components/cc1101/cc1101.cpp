...
std::vector<uint8_t> CC1101Component::receive_data(const uint8_t max_length) {
...
uint8_t CC1101Component::write_command_strobe(const uint8_t command) {
  uint8_t status_byte;

  this->select_();
  status_byte = this->transfer_byte(command | CC1101_WRITE_SINGLE);
  this->deselect_();

  return status_byte;
}

uint8_t CC1101Component::sidle() {
  uint8_t marcstate = 0xFF;
...
void CC1101Component::flush_rxfifo() {
  this->sidle();
  this->write_command_strobe(CC1101_SFRX);
  delayMicroseconds(100);
}

void CC1101Component::flush_txfifo() {
  this->sidle();
  this->write_command_strobe(CC1101_SFTX);
  delayMicroseconds(100);
}
...
void CC1101Component::write_register(const uint8_t address, const uint8_t data) {
  this->select_();
  this->write_byte(address | CC1101_WRITE_SINGLE);
  this->write_byte(data);
  this->deselect_();
}

uint8_t CC1101Component::read_register(const uint8_t address) {
  switch (address) {
    case CC1101_FREQEST:
...
void CC1101Component::write_burst_register(const uint8_t address, const std::vector<uint8_t> &data) {
  ...
  for (uint8_t out_byte : data) {
    this->write_byte(out_byte);
  }
  this->deselect_();
}

std::vector<uint8_t> CC1101Component::read_burst_register(const uint8_t address, const uint8_t max_length) {
  uint8_t count, i;
  std::vector<uint8_t> in_bytes;
  ...
  for (i = 0; i < count; i++) {
    in_bytes.push_back(this->transfer_byte(0x00));
  }
  this->deselect_();
...
uint8_t CC1101Component::read_register_(const uint8_t address) {
  uint8_t in_byte;
  ...
  in_byte = this->transfer_byte(0x00);
  this->deselect_();

  return in_byte;
}

uint8_t CC1101Component::read_register_with_sync_problem_(const uint8_t address) {
...
void CC1101Component::reset_(bool power_on_reset) {
  if (power_on_reset) {
...
int16_t CC1101Component::read_rssi() {
